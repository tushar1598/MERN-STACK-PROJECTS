...
export function AuthStudent({ children }) {
  const { data, authLoading } = useContext(UserContext);
  if (authLoading) return null;
  return data ? children : <Navigate to="/users/sign-in" />;
}
export function AuthCompany({ children }) {
  const { data, authLoading } = useContext(UserContext);
  if (authLoading) return null;
  return data ? children : <Navigate to="/users/sign-in" />;
}
export function AuthUpdateStudent({ children }) {
  const { data, authLoading } = useContext(UserContext);
  if (authLoading) return null;
  return data ? children : <Navigate to="/users/sign-in" />;
}
export function AuthDownloadStudent({ children }) {
  const { data, authLoading } = useContext(UserContext);
  if (authLoading) return null;
  return data ? children : <Navigate to="/users/sign-in" />;
}
